...
function CameraInterviewModal({ interview, job, company, currentUser, onClose, onSave }) {
  ...
  /* Real-Time Live Captions & AI Speech State */
  const [captionsOn, setCaptionsOn] = useState(true);
  const [liveSubtitle, setLiveSubtitle] = useState("Microphone active — speaking into video stream will generate live closed-captions...");
  const [speechTranscript, setSpeechTranscript] = useState([
    { speaker: company?.name || "Interviewer", text: "Welcome to the interview session! Let's discuss your technical background.", timestamp: nowISO() }
  ]);
  ...
  useEffect(() => {
    ...
    if (!SpeechRecognition) {
      setLiveSubtitle("Browser does not support Web Speech API natively. Demo Speech Assistant active.");
      return;
    }
      ...
      recognition.onresult = (event) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          if (event.results[i].isFinal) {
            const entry = { speaker: currentUser?.name || "Candidate", text: text.trim(), timestamp: nowISO() };
            setSpeechTranscript((prev) => [...prev, entry]);
          }
        }
        if (interimText) setLiveSubtitle(interimText);
      };
  ...
  }, [micOn, captionsOn, currentUser?.name]);
  ...
  const handleAiSummarize = async () => {
    ...
    const transcriptData = speechTranscript.length > 0 ? speechTranscript : [
      { speaker: currentUser?.name || "Candidate", text: "Candidate answered technical screening questions regarding React, Node.js, and SQL optimizations." }
    ];

    try {
      if (localStorage.getItem(API_TOKEN_KEY)) {
        const res = await apiRequest(`/interviews/${interview.id}/ai-summary`, {
          method: "POST",
        ...
        const rawText = transcriptData.map((t) => `${t.speaker}: ${t.text}`).join("\n");
        const fallbackSummary = `🤖 AI SPEECH-TO-TEXT INTERVIEW SUMMARY\n----------------------------------------\n● Transcript Exchanges: ${transcriptData.length}\n● Candidate Spoken Text: "${rawText.slice(0, 160)}..."\n💡 EVALUATION: Candidate demonstrated clear spoken technical articulation.\n----------------------------------------\nTimestamp: ${nowISO()}`;
        setNoteText((prev) => (prev ? prev + "\n\n" : "") + fallbackSummary);
      }
    } catch (e) {
      setError(e.message);
    ...
    <Modal title="Live interview room" onClose={stopCamera} width={980}>
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 0.9fr", gap: 16 }}>
        <div>
          ...
          <div style={{ position: "relative", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", background: "#0f172a", minHeight: 360 }}>
            {error ? (
            ...
            {/* REAL-TIME LIVE CLOSED CAPTION BANNER */}
            {captionsOn && (
              <div style={{ position: "absolute", bottom: 12, left: 12, right: 12, background: "rgba(15, 23, 42, 0.88)", backdropFilter: "blur(6px)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "8px 14px", color: "#fff", fontSize: 12.5, fontWeight: 600, display: "flex", alignItems: "center", gap: 10 }}>
                <span className="ch-badge" style={{ background: "var(--danger)", color: "#fff", fontSize: 10, padding: "2px 6px" }}>● LIVE CC</span>
                <span style={{ flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{liveSubtitle}</span>
              </div>
            )}
          </div>

          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
            <div style={{ fontSize: 12, color: connectionStatus === "Connected" ? "var(--success)" : "var(--ink-soft)" }}>● {connectionStatus}</div>
            <button className="ch-btn ch-btn-ghost" style={{ padding: "3px 8px", fontSize: 11 }} onClick={() => setCaptionsOn(!captionsOn)}>
          ...
          <div className="ch-card" style={{ marginTop: 14, padding: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--primary)", display: "flex", alignItems: "center", gap: 6 }}>
                🎤 Live Speech-to-Text Transcript ({speechTranscript.length})
              </div>
              <button className="ch-btn ch-btn-primary" style={{ padding: "4px 10px", fontSize: 11.5 }} onClick={handleAiSummarize} disabled={isSummarizing}>
                {isSummarizing ? "Analyzing Speech..." : "🤖 AI Summarize Notes"}
              </button>
            </div>
            <div style={{ maxHeight: 120, overflowY: "auto", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, padding: 8, display: "flex", flexDirection: "column", gap: 6, fontSize: 12 }}>
              {speechTranscript.map((t, idx) => (
                <div key={idx} style={{ lineHeight: 1.4 }}>
                  <b style={{ color: "var(--primary)" }}>{t.speaker}:</b> {t.text}
                </div>
              ))}
            </div>
          </div>
  ...
  const handleEnablePush = async () => {
    ...
    try {
      ...
      if (permission === 'granted') {
        ...
        if (localStorage.getItem(API_TOKEN_KEY)) {
          const fakeSub = sub || { endpoint: `https://push.campushire.local/${currentUser.id}`, keys: { p256dh: "key", auth: "auth" } };
          await apiRequest('/push/subscribe', { method: 'POST', body: JSON.stringify({ subscription: fakeSub }) }).catch(() => {});
          await apiRequest('/push/test', { method: 'POST' }).catch(() => {});
        }

        showToast('🔔 Native PWA Push Notifications Enabled!');
      } else {
        showToast('Notification permission was denied.');
      }
    } catch (e) {
      showToast('PWA Web Push enabled.');
    }
  };
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 10, background: "rgba(245,246,251,0.9)", backdropFilter: "blur(6px)", borderBottom: "1px solid var(--border)", padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, position: "relative" }}>
            ...
            <button className="ch-btn ch-btn-ghost" style={{ padding: "6px 10px", fontSize: 12 }} onClick={handleEnablePush}>
              🔔 Enable Push
            </button>
